...
urlpatterns = [
    ...
    url(r'^refs/page/(?P<page_no>[0-9]+)/$', references.ref_index, name='ref_index_paged'),

    url(r'^login/$', login, name='login'),
    url(r'^logout/$', logout, name='logout'),

    url(r'^refs/new/$', references.ref_edit, name='ref_create'),
    url(r'^refs/edit/(?P<ref_id>[0-9]+)/$', references.ref_edit, name='ref_edit'),
    url(r'^refs/(?P<ref_id>[0-9]+)/$', references.ref_detail, name='ref_detail'),
    url(r'^refs/(?P<ref_id>[0-9]+)/vote/(?P<vote_type>(up|down))/$', references.ref_vote, name='ref_vote'),

    url(r'^tags/$', lambda req: tags.tag_index(req, 1), name='tag_index'),
    url(r'^tags/page/(?P<page_no>[0-9]+)/$', tags.tag_index, name='tag_index_paged'),
